...
print("=== ENHANCED ACCURATE ESCALATION CHECK FOR JULY 20 ===")
unsubmitted = {}
for r in reminders:
    ...
    for raw_msg in raw_msgs_july20:
        ...
        # Broad matching for person/group
        if match_sender_raw or match_group_raw or match_name:
            matched_msgs.append(raw_msg)
        # Also include messages from Mahalakshmi if reminder is assigned to Mahalakshmi
    ...
    for m in matched_msgs:
        ...
        if is_egg_pricing:
            ...
            # Check time slot or keywords
            is_time_match = False
            if time_keyword == 'morning' and (msg_hour < 12 or 'morning' in text_lower or '7:' in text_lower or '8:' in text_lower or '9:' in text_lower or '10:' in text_lower):
                is_time_match = True
            ...
            if is_time_match and any(w in text_lower for w in ['egg', 'price', 'pricing', 'ppr rate', 'closing', 'veh kol', 'papaak']):
                submitted = True
