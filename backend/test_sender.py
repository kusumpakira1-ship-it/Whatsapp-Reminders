...
msgs = db.query(RawMessage).filter(RawMessage.sender.like('%7259510983%')).order_by(RawMessage.timestamp.desc()).limit(5).all()
with open("sender_output.txt", "w", encoding="utf-8") as f:
    for m in msgs:
