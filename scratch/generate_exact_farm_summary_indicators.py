"""
Generate exact Daily Farm Summary for Yesterday (13 Aug 2026) with Green/Red indicators and Shead labels.
"""
...
for s_num in range(1, 9):
    ...
    prod_icon = "🟢" if actual_pct >= book_pct else "🔴"
    trays_fmt = f"{trays}.{loose:02d}"
    production_rows.append(f"{sname}: {age_weeks}w | {trays_fmt} Trays | {actual_pct}% (Book: {book_pct}%) {prod_icon}")
