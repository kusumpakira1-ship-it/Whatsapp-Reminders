    ...
    for jid in jids:
        ...
        try:
            ...
            if resp.status_code != 200:
                print(f"Error fetching {jid}: status {resp.status_code}")
                continue
            msgs = resp.json()
            for m in msgs:
                ts = m.get('timestamp')
            ...
            print(f"Fetch exception for {jid}: {err}")

db.commit()
print(f"Synced {inserted_count} new raw messages into SQLite database.")

# Run evaluation
data = evaluate_attendance_for_date(today_str)

print("\n" + "="*80)
print(f"STRICT GROUP-SCOPED LOGOUT & ATTENDANCE AUDIT FOR ALL 31 EMPLOYEES ({today_str}):")
print("="*80 + "\n")
