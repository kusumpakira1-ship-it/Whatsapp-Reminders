"""
Generate exact Daily Farm Summary for Yesterday (13 Aug 2026) using supervisor data and book standards DB.
"""
...
# 2. Production Data for 13 Aug 2026 from supervisor endpoint
# (Shead: Trays, Loose, Total Eggs, Damaged)
production_data = {
...
# 3. Birds Weight Data from supervisor endpoint
weight_data = {
    'Shead 1': 1417.88,
    'Shead 2': 1412.38,
    'Shead 3': 1235.63,
...
for s_num in range(1, 9):
    ...
    # Get book standard for age_weeks
    ...
    actual_pct = round((total_eggs / live_birds * 100)) if live_birds > 0 else 0
    
    trays_fmt = f"{trays}.{loose:02d}"
    production_rows.append(f"{s_num}._ {age_weeks}._ {trays_fmt}_{actual_pct}%-{book_pct}%")
    ...
    if act_weight:
        ...
        weight_rows.append(f"Shead {s_num}: {act_weight:.2f}g (Book: {book_weight}g | {diff_str})")
    else:
        weight_rows.append(f"Shead {s_num}: No Weight Data (Book: {book_weight}g)")
...
💀 *Shed-Wise Mortality*
```
1-{mortality_data['Shead 1']}
2-{mortality_data['Shead 2']}
3-{mortality_data['Shead 3']}
4-{mortality_data['Shead 4']}
5-{mortality_data['Shead 5']}
6-{mortality_data['Shead 6']}
7-{mortality_data['Shead 7']}
8-{mortality_data['Shead 8']}
Chick 1: {mortality_data['Chick 1']}
...
⚖️ *Birds Weight Comparison (Actual vs Standard Book)*
```
""" + "\n".join(weight_rows) + """
```"""
