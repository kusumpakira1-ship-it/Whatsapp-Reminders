"""
Test Option B 9:30 PM escalation message generator
"""
...
def test_option_b_generator():
    ...
    for title, items in sections_config:
        ...
        total_missing_count += len(missing_items)
        if missing_items:
            lines_930 = [f"{title}"]
    ...
    if total_missing_count == 0:
        messages_930 = [f"🎉 *Daily Escalation Report*\n📅 *Date:* {today_date_str}\n\nAll reports and tasks have been submitted successfully today! ✅"]

    return messages_930

msgs = test_option_b_generator()
print(f"Option B messages to send at 9:30 PM ({len(msgs)} total):")
