...
for grp_name, jids in groups:
    for jid in jids:
        try:
            ...
            resp = requests.get(url, headers={'X-Api-Key': '123'}, timeout=5)
...
db.commit()
print(f"Synced {inserted_count} new messages into SQLite.")

data = evaluate_attendance_for_date(today_str)

print("\n" + "="*80)
print(f"AUDIT OF ALL 31 EMPLOYEES LOGOUT / SIGN-OFF STATUS FOR TODAY ({today_str}):")
print("="*80 + "\n")
...
for grp in data['groups']:
    ...
    for emp in grp['employees']:
        ...
        detail = emp['detail']
        print(f"  • {ename:<12} | Login: {ltime:<10} | Logout: {otime:<10} | Status: {badge:<32} | Detail: {detail}")
    print("-" * 80)
