...
def main():
    ...
    with engine.begin() as con:
        try:
            ...
            sql = text(f"""
                ALTER TABLE sunfra_processed_data
                MODIFY COLUMN category ENUM({NEW_ENUM}) NOT NULL DEFAULT 'unknown';
            """)
            con.execute(sql)
            print("✅ Successfully expanded category ENUM in sunfra_processed_data table.")
        ...
        try:
            ...
            con.execute(text(
                "ALTER TABLE sunfra_processed_data ADD COLUMN IF NOT EXISTS group_name VARCHAR(255);"
            ))
