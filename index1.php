...
if (isset($_GET['api'])) {
    ...
    elseif ($action === 'add_device' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $power = strtoupper(trim($input['power_status'] ?? 'ON'));
    ...
    elseif ($action === 'simulate' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        echo json_encode(['status' => 'success', 'message' => 'Simulated discrete telemetry (0%, 25%, 50%, 75%, 100%) successfully']);
        exit;
    }
}
?>
  ...
  <div class="metrics-grid">
    <div class="metric-card blue">
      <div class="metric-info">
        ...
        <div class="metric-value" id="stat-total">2</div>
      </div>
    ...
    <div class="metric-card green">
      <div class="metric-info">
        ...
        <div class="metric-value" id="stat-on">2</div>
      </div>
      <div class="metric-icon">⚡</div>
    </div>
...
<script>
  ...
  async function loadDevices() {
    try {
      const res = await fetch('index.php?api=get_devices');
  ...
  function renderDevices(filterMac = '') {
    ...
    filtered.forEach(d => {
      ...
      tr.innerHTML = `
        ...
        <td>
          <button class="btn-action edit" onclick="alert('Editing ${d.mac_address}')">✏️ Edit</button>
  ...
  async function saveDevice(e) {
    ...
    try {
      await fetch('index.php?api=add_device', {
        method: 'POST',
  ...
  async function simulateTelemetry() {
    ...
    devicesData.forEach(d => d.last_seen = nowStr);
    try {
      await fetch('index.php?api=simulate', { method: 'POST' });
    } catch(e) {}
    renderDevices();
  ...
  function deleteDevice(mac) {
    if (confirm(`Are you sure you want to delete MAC Device ${mac}?`)) {
      devicesData = devicesData.filter(d => d.mac_address !== mac);
      renderDevices();
    }
  }
  ...
  loadDevices();
  setInterval(loadDevices, 10000); // Auto-update live telemetry data every 10 seconds
</script>
</body>
</html>
